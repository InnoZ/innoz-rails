$(document).ready(function(){
	...
	displayHelper._addOption = function(option){
		...
		if(option.length > 0){
			$('<tr class="new"><td class="index"></td><td class="option"><h4>' + option + '</h4></td><td></td></tr>')
	...
	displayHelper._updateTableIndex = function(){
		$('#optionsList .index').each(function(index){
			$(this).html('<strong>Topic ' + (index+1) + ': </strong>');
		});
	};
	...
	ahp._displayResults = function(calcResults){
		...
		for(var i = 0; i < ahp.optionArray.length; i++){
			...
			html += '<tr class="result_set" >' +
					...
					'<td>' +
					// bar
					'<div style="background-color:steelblue;width:' + (resultText * 1.2 * 100).toFixed(4) + 'px">&nbsp;</div>' +
					'</td>' +
					'</tr>';
		}
		// the question text
